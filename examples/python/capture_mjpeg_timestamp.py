...
#  capture video in MJPEG format with timestamp
#  
...
def main(args):
    ...
    config_video = myCamera.create_video_configuration(main={"size": (1920, 1080)})
    myCamera.configure(config_video)

    myCamera.start_preview(Preview.QTGL)
    encoder = JpegEncoder(q=70)

    myCamera.start_recording(encoder, "capture_mjpeg_timestamp.mjpeg", pts="timestamp.txt")
    time.sleep(10)
